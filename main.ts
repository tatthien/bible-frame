        ...
        async (source, el, ctx) => {
          try {
            ...
            const res = await fetch(
              `${BASE_API_URL}/verses?address=${encodeURIComponent(source)}`,
            )
            const json = await res.json()

            const scriptureEl = rootEl.createEl('div', {
              cls: 'bible-frame-scripture',
            })
            scriptureEl.innerHTML = json.length
              ? json
                  .map(
                    (verse: any) =>
                      `<sup style="font-size: 0.5em; font-weight: 500">${verse.number}</sup> ${verse.content}`,
                  )
                  .join(' ')
            ...
            if (json.length) {
              ...
              MarkdownRenderer.render(
                this.app,
                `[[${book.toUpperCase()}-${chapter}#${verses[0]} | ${capitalize(source)}]]`,
                srcEl,
                ctx.sourcePath,
                this,
              )
